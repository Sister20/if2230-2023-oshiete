        ...
        for (int i = 1; i < (int)(CLUSTER_SIZE / sizeof(struct FAT32DirectoryEntry)); i++)
        {
            ...
            if (current_entry_has_same_extension && current_entry_has_same_name && current_entry_is_undelete)
            {
                ...
                if (current_entry_is_folder)
                {
                    ...
                    {
//////////////////////////
                        // check if there are any objects in current directory
                        int dir_table_length1 = (int)CLUSTER_SIZE / sizeof(struct FAT32DirectoryEntry);
                        int currentClusterNumber2 = dir_cluster_number;
                        bool check_all_dir_table1 = 0;
                        bool not_empty1 = 0;

                        while (!check_all_dir_table1)
                        {
                            for (int i = 1; i < dir_table_length1; i++)
                            {
                                // delete each entry
                                // found a file
                                if (searched_dir.table[i].undelete)
                                {
                                    ...
                                    struct FAT32DriverRequest req = {
                                        ...
                                        .ext = "\0\0\0",
                                        .parent_cluster_number = currentClusterNumber2 ,
                                        .buffer_size = 8 * CLUSTER_SIZE,
                                    };

                                    delete_all(req);
                                    check_all_dir_table1 = 1;
                                    not_empty1 = 1;
                                    break;
                                }

                            }
                            if (driver_state.fat_table.cluster_map[currentClusterNumber2] != FAT32_FAT_END_OF_FILE && !not_empty1)
                            {
                                read_clusters(&searched_dir, driver_state.fat_table.cluster_map[currentClusterNumber2], 1);
                                currentClusterNumber2 = driver_state.fat_table.cluster_map[currentClusterNumber2];
                                check_all_dir_table1 = 0;
                            }
                            else if (driver_state.fat_table.cluster_map[currentClusterNumber2] == FAT32_FAT_END_OF_FILE)
                            {
                                check_all_dir_table1 = 1;
                            }
